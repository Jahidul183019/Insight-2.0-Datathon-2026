"""Build the 90%-confidence pseudo-label candidate for Submission 11.

This is a controlled follow-up to Submission 6.  It uses Submission 6's
pre-pseudo teacher probabilities, changes only the confidence gate from
95%/5% to 90%/10%, retrains the same six-model student, blends teacher and
student 50/50, and preserves Submission 6's exact positive count.

The script never overwrites the canonical ``submission.csv``.  Outputs are
isolated under ``artifacts/pseudo90/`` until the validation gate is passed.
"""
...
np.random.seed(SEED)
SMOOTHING_M = 20.0
OUTPUT_DIR = Path("artifacts/pseudo90")
TEACHER_PROBS_PATH = Path("archive/probs_v6_blend.npy")
BASELINE_SUBMISSION_PATH = Path("submission6.csv")

print("=" * 80)
print("  SUBMISSION 11 CANDIDATE: 90% PSEUDO-LABELING")
print("=" * 80)
...
# Load Submission 6's pre-pseudo teacher probabilities.  Using the final v6
# probabilities here would turn this into iterative pseudo-labeling (the
# already-tested Submission 9 recipe), rather than a clean threshold test.
print("\n>>> Loading Submission 6 pre-pseudo teacher probabilities...")
test_blend = np.load(TEACHER_PROBS_PATH, allow_pickle=False)
baseline_submission = pd.read_csv(BASELINE_SUBMISSION_PATH)
if len(test_blend) != len(test_df) or not np.isfinite(test_blend).all():
    raise ValueError("Teacher probabilities are invalid or misaligned")
if baseline_submission.columns.tolist() != ["patient_id", "vital_status"]:
    raise ValueError("Submission 6 has an invalid schema")
if not baseline_submission["patient_id"].equals(test_df["patient_id"]):
    raise ValueError("Submission 6 IDs/order do not match test.csv")
target_dead_count = int((baseline_submission["vital_status"] == "Dead").sum())
...
# Frozen Submission 6 blend selected from its OOF sweep.
best_alpha = 0.55

# ============================================================================
# PHASE 1: One-pass pseudo-labeling at 90% confidence
# ============================================================================
print("\n>>> Pseudo-Labeling at 90% confidence")
...
# Match Submission 6: 50% original pre-pseudo teacher + 50% pseudo student.
final_ens = 0.5 * test_blend + 0.5 * ps_blend

# ============================================================================
# PHASE 2: Generate isolated candidate artifacts
# ============================================================================
print("\n>>> Phase 2: Generating deterministic top-k candidate")

order = np.argsort(final_ens, kind="mergesort")
preds = np.zeros(len(final_ens), dtype=np.int8)
preds[order[-target_dead_count:]] = 1
sub = pd.DataFrame({"patient_id": test_ids, "vital_status": np.where(preds==1,"Dead","Alive")})
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
np.save(OUTPUT_DIR / "probs_pseudo90.npy", final_ens)
sub.to_csv(OUTPUT_DIR / "submission11_pseudo90.csv", index=False)
dead_n = preds.sum()
baseline_labels = (baseline_submission["vital_status"] == "Dead").to_numpy()
changed = int((preds.astype(bool) != baseline_labels).sum())
print(f"  Candidate: {OUTPUT_DIR / 'submission11_pseudo90.csv'}")
print(f"  Probabilities: {OUTPUT_DIR / 'probs_pseudo90.npy'}")
print(f"  Dead={dead_n:,} ({preds.mean():.3%}); changed vs Sub 6={changed:,}")

print(f"\n{'='*80}")
print(f"  PSEUDO90 CANDIDATE COMPLETE")
print("  Canonical submission.csv was not touched")
print(f"{'='*80}")
