...
SMOOTHING_M = 20.0
LEAKAGE_CLEAN = False
OUTPUT_DIR = Path("artifacts/v9_harmonized")
...
def build_features(df):
    ...
    F["tumor_size_harmonized"] = F["tumor_size_sm"].fillna(F["cs_tumor_size"]).fillna(F["tumor_size_ot"])
    F["tumor_size_missing"] = F["tumor_size_harmonized"].isna().astype(int)
    
    # Surgery Harmonization
    def _surg(v):
        if pd.isna(v): return np.nan
        s = str(v).strip()
        if s == "Blank(s)" or s.startswith("Unknown"): return np.nan
        try: return float(s)
        except: return np.nan
    surg_98 = df["rx_summ_surgprim_site19982022"].apply(_surg)
    surg_23 = df["rx_summ_surgprim_site20232023"].apply(_surg)
    F["surgery_harmonized"] = surg_98.fillna(surg_23)
    ...
    comp_stage = derive_composite_stage(df)
    F["is_localized_harmonized"] = ((df["summary_stage"]=="Localized") | (comp_stage == "M0 N0 T0-2 localized")).astype(int)
...
best_base_oof = best_alpha * oof_v5 + (1 - best_alpha) * oof_v4
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
np.save(OUTPUT_DIR / "probs_v9_base_oof.npy", best_base_oof)
